...
- Oportunidades de arbitragem (YES+NO < $1)
- Ordens abertas

## ESTRATÉGIAS DE LUCRO (use TODAS):

### 1. ARBITRAGEM YES+NO (PRIORIDADE MÁXIMA - RISCO ZERO)
- Se o preço de YES + NO < $1.00 no mesmo mercado, compre AMBOS
- Lucro garantido = $1.00 - (preço_yes + preço_no)
- Ex: YES=0.45, NO=0.50 → custo=0.95, lucro=$0.05 por par (5.3% risk-free)
- SEMPRE execute arbitragens quando disponíveis - é dinheiro grátis
- Os mercados com "arbitrage_spread" > 0 já foram identificados pra você

### 2. APOSTAS DE VALOR (HIGH CONVICTION)
- Compre YES baratos em eventos que provavelmente VÃO acontecer
- Compre NO baratos em eventos improváveis que o mercado superestima
- 70% dos mercados resolvem NO - apostar contra hype é lucrativo
- Foque em mercados com volume alto (melhor liquidez)

### 3. GESTÃO DE PORTFÓLIO
- Venda posições perdedoras RÁPIDO (cut losses)
- Deixe vencedoras correr
- Diversifique entre 3-5 temas diferentes

### 4. TIMING
- Compre contrarian quando o mercado está em pânico
- Venda quando todos estão eufóricos
- Eventos próximos do vencimento com preço distorcido = oportunidade

REGRAS:
- SEMPRE execute arbitragens quando spread > 1%
- Seja agressivo - o objetivo é multiplicar o dinheiro
- Para vendas, só venda posições que já existem
- Use USDC values inteiros ($1, $2, $5) para amount
- NUNCA gaste mais que 40% do saldo em um único trade
...
- Se o outcome acontecer, cada share vale $1.00, então lucro = (1.0 - price) × shares

Responda com APENAS JSON válido neste formato:
{
  "analysis": "Análise breve do mercado e raciocínio (2-3 frases em português)",
  "trades": [
    {
      "action": "BUY" ou "SELL",
...
class AutoTrader:
    ...
    def __init__(self, agent):
        ...
        # Config
        self.interval_minutes = int(os.getenv("AUTOTRADE_INTERVAL_MIN", "30"))
        self.max_trade_amount = float(os.getenv("AUTOTRADE_MAX_AMOUNT", "25"))
        self.max_trades_per_cycle = int(os.getenv("AUTOTRADE_MAX_TRADES", "3"))
    ...
    def _gather_context_sync(self) -> dict:
        """Gather current market state for the LLM to analyze."""
        context = {
        ...
        # Markets (top 30 by volume for best liquidity)
        try:
    ...
    async def run_cycle(self) -> str:
        ...
        try:
            ...
            logger.info(f"Context gathered: {len(context.get('markets', []))} markets, {len(context.get('positions', []))} positions")
        except Exception as e:
        ...
        # Extra instructions for goal mode
        extra = ""
        if self.goal_mode:
            portfolio_value = await self._get_portfolio_value()
        ...
        # Ask LLM
        try:
            logger.info("Asking xAI for trade recommendations...")
            recommendations = await self._ask_llm_for_trades(context, extra)
            logger.info(f"xAI response: {len(recommendations.get('trades', []))} trades recommended")
        except Exception as e:
