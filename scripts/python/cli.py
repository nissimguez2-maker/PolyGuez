...
def query_local_markets_rag(vector_db_directory: str, query: str) -> None:
    """Query a local RAG database of Polymarket events."""
...
def ask_superforecaster(event_title: str, market_question: str, outcome: str) -> None:
    ...
    print(
        "event: str = {event_title}, "
        "question: str = {market_question}, "
        "outcome (usually yes or no): str = {outcome}"
    )
...
def ask_llm(user_input: str) -> None:
    """Ask a generic question to the LLM."""
    executor = Executor()
...
def ask_polymarket_llm(user_input: str) -> None:
    """Ask the LLM about markets you want to trade."""
    executor = Executor()
