"""PolyGuez domain models — config, signal state, trade records, rolling stats.

Polymarket API response types (Trade, Market, SimpleMarket, etc.) have been
moved to agents.utils.polymarket_types but are re-exported here so all
existing `from agents.utils.objects import Market` calls continue to work.
"""
...
# Re-export Polymarket API types for backwards compatibility
from agents.utils.polymarket_types import (  # noqa: F401
    Article,
    ClobReward,
    ComplexMarket,
    Market,
...
class PolyGuezConfig(BaseModel):
    ...
    max_daily_loss: Optional[float] = Field(default=20.0)
