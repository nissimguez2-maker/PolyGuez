...
def implied_vol(token_price: float, spot: float, strike: float,
                                seconds_remaining: float, sigma_hint: float | None = None) -> float | None:
                                      """
    ...
    def bs_digital(sigma):
        ...
        return norm.cdf(d2) - token_price

    try:
              return brentq(bs_digital, 0.01, 10.0, xtol=1e-4, maxiter=50)
